...
CStateFalmerOH_Attack2::CStateFalmerOH_Attack2()
{
}

HRESULT CStateFalmerOH_Attack2::Initialize(CGameObject* _pMonster, CGameObject* _pPlayer, CTransform* _pMonsterTransform, CNavigation* _pMonsterNavigation, vector<CCollider*> _pVecColCom)
{
...
void CStateFalmerOH_Attack2::Update(_float _fTimeDelta)
{
		...
		m_pPlayer->Get_CurState() == CPlayer::ONEHAND_ANTICIPATE)
	{
		if (pGameInstance->Collision_Enter(m_pWeaponCollider, m_pPlayerWeaponCollider))
		{
			...
			m_pMonster->Set_State(CFalmer_OneHand::FALMEROH_STAGGERL2);
...
void CStateFalmerOH_Attack2::Late_Update(_float _fTimeDelta)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
CStateFalmerOH_Attack2* CStateFalmerOH_Attack2::Create(CGameObject* _pMonster, CGameObject* _pPlayer, CTransform* _pMonsterTransform, CNavigation* _pMonsterNavigation, vector<CCollider*> _pVecColCom)
{
	CStateFalmerOH_Attack2* pInstance = new CStateFalmerOH_Attack2();

	if (FAILED(pInstance->Initialize(_pMonster, _pPlayer, _pMonsterTransform, _pMonsterNavigation, _pVecColCom)))
	{
		MSG_BOX("Fail Create : CStateFalmerOH_Attack2");
...
void CStateFalmerOH_Attack2::Free()
{
}
