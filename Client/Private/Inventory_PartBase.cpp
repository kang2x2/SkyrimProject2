...
HRESULT CInventory_PartBase::Initialize_Clone(void* _pArg)
{
	...
	m_pTransformBlack->Set_Scaling(_float3(m_fSizeX, m_fSizeY, 1.f));
	m_pTransformBlack->Set_State(CTransform::STATE_POSITION,
		XMVectorSet(m_vBlackPos.x - m_vBlackSize.x * 0.5f, 
					-m_vBlackPos.y + m_vBlackSize.y * 0.5f, 0.f, 1.f));

	m_pTransformLine1->Set_Scaling(_float3(m_fSizeX, m_fSizeY, 1.f));
	m_pTransformLine1->Set_State(CTransform::STATE_POSITION,
		XMVectorSet(m_vLine1Pos.x - m_vLine1Size.x * 0.5f,
					-m_vLine1Pos.y + m_vLine1Size.y * 0.5f, 0.f, 1.f));

	m_pTransformLine2->Set_Scaling(_float3(m_fSizeX, m_fSizeY, 1.f));
	m_pTransformLine2->Set_State(CTransform::STATE_POSITION,
		XMVectorSet(m_vLine2Pos.x - m_vLine2Size.x * 0.5f,
