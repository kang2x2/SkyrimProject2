...
void CStatePlayerOH_LAttackB::Update(_float _fTimeDelta)
{
	...
	if (dynamic_cast<CPlayer*>(m_pPlayer)->Get_CurFrameIndex() >= 25 &&
		dynamic_cast<CPlayer*>(m_pPlayer)->Get_CurAnimationName("1hm_walkbwdattackleft") &&
		pGameInstance->Get_DIMouseDown(CInput_Device::MKS_LBUTTON))
	{
		dynamic_cast<CPlayer*>(m_pPlayer)->Set_State(CPlayer::ONEHAND_RATTACKB);
		dynamic_cast<CPlayer*>(m_pPlayer)->Play_Animation(false, "1hm_walkbwdattackright");
	}
